...
export default function Team() {
  ...
  return (
    <section id='leadership-team' className="w-full flex flex-col gap-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 px-6 md:px-0">
        {otherMembers.map((member) => (
          <div
            key={member.name}
            className="bg-background rounded-xs overflow-hidden row-span-1 group">
            {/* Member Image */}
            <Image
              src={member.image}
              alt={member.name}
